...
def get_answer_from_glm(tokenizer, model, dynasty, cid, passage, question, options):
    ...
    response, history = model.chat(tokenizer, user_request, history=message)
...
def load_model(model_path, device_id=None):
    ...
    if device_id is not None:
        ...
        model = model.cuda()  # Load to the default GPU
    model.eval()
...
def fewshot_learning(source_path, log_path):
    ...
    tokenizer, model = load_model("/disk2/huanggj/chatGLM_demo/chatglm2-6b-32k")

    if len(cids) > 0:
        for index, cid in enumerate(cids):
            data  = datas_map.get(cid)
            answer = get_answer_from_glm(tokenizer, model, data['dynasty-glm'], cid, data['context'], data['qas'][0]['question'], data['qas'][0]['options'])
            answer.replace("\n", "")
...
if __name__ == '__main__':
    ...
    test_log_path = "./log_glm_20240315.txt"


    fewshot_learning(test_source_path, test_log_path)
