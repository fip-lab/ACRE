...
desc : topk 加上下句(剩下的句子，对与每个输入分支获取3句与其他3个选项相似度最低的句子，也就是每个分支补充一些不那么相关的信息)
...
class Input_Preparation:
    ...
    def assign_remaining_sentences(self, options, sentence_embeddings, remaining_sentences_indices):
        assigned_remaining_sentences = defaultdict(list)

        for i in range(len(options)):
            current_option = options[i]
            other_options = [options[j] for j in range(len(options)) if j != i]
            ...
            for remaining_sentence_index in remaining_sentences_indices:
                ...
                # 计算与其余选项的相似度得分之和
                similarity_sum = sum(cosine_similarity(self.option_embeddings[o_p], sentence_embeddings[remaining_sentence_index])
                    for o_p in other_options
                )

                score_list.append(similarity_sum)

            idx_list = get_lowest_three_indexes(score_list)

            indices = [ remaining_sentences_indices[idx]  for idx in idx_list]

            assigned_remaining_sentences[options[i]].append(indices)
    ...
    def prepare_inputs(self, options, all_sentences, question, n):
        ...
        for option in options:
            ...
            self.top_sentences[option] = top_indices
        ...
        assigned_remaining_sentences = self.assign_remaining_sentences(options, all_sentences_embeddings,
...
def get_lowest_three_indexes(scores):
    # 使用enumerate函数获取分数及其索引，并按分数进行排序
    sorted_scores = sorted(enumerate(scores), key=lambda x: x[1])
    # 获取最低的三个分数的索引
    lowest_three_indexes = [index for index, _ in sorted_scores[:3]]
    return lowest_three_indexes
...
def evidence_extract(n, source_path):
    des_path = f"./top{n}_evidence_extract_low3.json"
